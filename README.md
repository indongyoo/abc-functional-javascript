...
## 특징
  ...
  - 비동기 제어 기능과 새로운 인자 패턴을 가진 `each`, `map`, `reduce`, `filter`, `reject`, `find`, `some`, `every`, `uniq` 함수가 있습니다.
  - Jade와 비슷한 문법, Function을 쉽게 사용할 수 있는 HTML Template Engine.
  - sql 등을 작성하기 편한 함수가 있습니다.
  - 다른 자바스크립트 라이브러리에 대한 의존성이 없는 750줄의 작은 라이브러리.
...
메서드를 통해 클래스 내부에서만 값을 참조하는 것보다 쉽고 유연하고 추상적이지 않은 방법이 있습니다.
